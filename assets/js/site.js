...
(function () {
  ...
  var now = new Date();
  var key = NAMES[now.getDay()];
  var span = CONFIG.hours[key];
  var dot = document.getElementById("live-dot");
  var text = document.getElementById("live-text");
  var hrs = document.getElementById("today-hours");
  var note = document.getElementById("today-note");
  var cell = document.querySelector('.wday[data-day="' + key + '"]');
  if (cell) cell.classList.add("now");
  if (!span) {
    if (dot) dot.className = "dot off";
    if (text) text.innerHTML = '<span class="txt">Closed </span>today';
    if (hrs) hrs.textContent = "Closed today";
    if (note) note.textContent = "Pick any other day and we will hold the room.";
  } else {
    var mins = function (t) { return Number(t.slice(0, 2)) * 60 + Number(t.slice(3)); };
    var nowM = now.getHours() * 60 + now.getMinutes();
    var live = nowM >= mins(span.open) && nowM < mins(span.close);
    if (hrs) hrs.textContent = span.open + " to " + span.close;
    if (dot) dot.className = live ? "dot" : "dot off";
    if (text) {
      text.innerHTML = live
        ? '<span class="txt">Open until </span>' + span.close
        : '<span class="txt">Opens </span>' + span.open;
    }
    if (note) {
      note.textContent = live
        ? LONG[now.getDay()] + ", reception is answering now."
        : LONG[now.getDay()] + ", bookings still go through.";
    }
  }
  ...
  var io = new IntersectionObserver(function (rows) {
    rows.forEach(function (row) {
      if (row.isIntersecting) { row.target.classList.add("in"); io.unobserve(row.target); }
    });
